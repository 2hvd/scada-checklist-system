...
(function () {
    ...
    // Global page-reload change polling (separate from dashboard list polling) defaults to 2s.
    // Use 5-10s for larger deployments (~50+ concurrent users).
    const POLL_INTERVAL_MS = readPositiveNumber(body.dataset.autoRefreshMs, 2000, 'data-auto-refresh-ms');
    // Short guard to avoid refreshing exactly while a click/typing action is being processed.
    const MIN_IDLE_MS = readPositiveNumber(body.dataset.autoRefreshIdleMs, 500, 'data-auto-refresh-idle-ms');
    ...
    async function checkForChanges() {
        ...
        try {
            ...
            console.warn('Real-time refresh poll failed; retrying on next interval.', err);
        } finally {
            inFlight = false;
        }
    }
