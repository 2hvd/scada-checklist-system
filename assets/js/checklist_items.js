    ...
    _renderTable(items, tbody) {
        ...
        tbody.innerHTML = items.map(item => {
            ...
            const usageBadge = usageCount > 0
                ? `<span class="badge badge-ci-usage" title="Used in ${usageCount} SWO(s)" aria-label="Warning: used in ${usageCount} SWO${usageCount > 1 ? 's' : ''}">⚠ ${usageCount} SWO${usageCount > 1 ? 's' : ''}</span>`
