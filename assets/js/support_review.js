/* support_review.js - Full-page support review with auto-save */

const SupportReview = {
    swoId: null,
    saveTimers: {},
    overallSaveTimer: null,
    ...
    async load() {
        try {
            const data = await API.get('/swo/get_support_review.php', { swo_id: this.swoId });
            ...
            console.error('SupportReview.load error:', err);
            document.getElementById('reviewContent').innerHTML =
    ...
    renderPage(data) {
        ...
        // Header
        document.getElementById('reviewTitle').textContent = 'Review: ' + swo.swo_number;
                    ...
                    <tbody>
                        ...
                        <tr data-item-key="${escapeHtml(item.key)}">
                            ...
                            <td class="col-comment">
                                <textarea class="review-comment-textarea support-comment-textarea"
                                          data-item-key="${escapeHtml(item.key)}"
                                          rows="2"
    ...
    attachEventListeners() {
        ...
        content.addEventListener('change', (e) => {
            if (e.target.classList.contains('support-decision-select')) {
        ...
        content.addEventListener('blur', (e) => {
            if (e.target.classList.contains('support-comment-textarea')) {
                const itemKey = e.target.dataset.itemKey;
                this.scheduleItemSave(itemKey);
            }
        }, true);
    ...
    async saveItemReview(itemKey) {
        ...
        try {
            const data = await API.post('/swo/support_item_review.php', {
                swo_id:           this.swoId,
                item_key:         itemKey,
    ...
    async saveOverallComments() {
        ...
        this.showSaving();
        try {
            const data = await API.post('/swo/support_overall_review.php', {
                swo_id:           this.swoId,
    ...
    async rejectAndSendToUser() {
        ...
        if (!comments) {
            showWarning('Please add overall comments with the rejection reason before rejecting.');
