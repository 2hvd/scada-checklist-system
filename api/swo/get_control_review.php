...
foreach ($bySection as $secKey => $rows) {
    ...
    foreach ($allItemRows as $row) {
        if (intval($row['visible_control'] ?? 1) === 1) continue;
...
if ($snapCnt > 0) {
    $summaryStmt = $conn->prepare(
        "SELECT ci.id, ci.item_key, ci.parent_item_id, ci.control_parent_item_id
         FROM swo_checklist_items sci
         JOIN checklist_items ci ON ci.id = sci.item_id
         WHERE sci.swo_id = ? AND COALESCE(ci.visible_control, 1) = 1"
    );
...
foreach ($summaryRows as $row) {
    $effectiveParent = $row['control_parent_item_id'] !== null
        ? intval($row['control_parent_item_id'])
        : ($row['parent_item_id'] !== null ? intval($row['parent_item_id']) : null);
...
foreach ($summaryRows as $row) {
    ...
    $status = $controlReviews[$row['item_key']]['decision'] ?? 'empty';
    $totalItems++;
    switch ($status) {
        case 'done':    $doneCount++;    break;
        case 'na':      $naCount++;      break;
